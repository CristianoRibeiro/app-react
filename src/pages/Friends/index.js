...
export default function Main(props) {
  ...
  async function _getData() {
    setLoading(true);
    try {
      const response = await api.post('/api/friends/all', {event_id: event.id});
  ...
  async function _scanner(value) {
    ...
    try {
      let response = await api.post('/api/set/friends', {
        code: value.data,
        event_id: event.id,
      });
      ...
      <Card>
        ...
        <View
          ...
          <View style={{flex: 1}}>
            ...
            <SubTitle style={{fontWeight: '300'}}>{item.friend.email}</SubTitle>

            <SubTitle style={{fontWeight: '700'}}>{item.friend.apcef}</SubTitle>
            {item.event ? (
              <SubTitle style={{fontWeight: '400', fontSize: 12}}>
                Juntos em:{' '}
                <SubTitle style={{fontWeight: '700', fontSize: 12}}>
                  {item.event.name}
                </SubTitle>
              </SubTitle>
            ) : null}
