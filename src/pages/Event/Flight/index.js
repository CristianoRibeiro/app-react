...
export default function Main(props) {
  ...
  async function _getData() {
    setLoading(true);
    try {
      let response = await api.post('/api/events/flights', {
        event_id: event.id,
      });
      ...
      await dispatch({type: 'FLIGHTS', payload: response.data});
  ...
  function _openUrl(item) {
    try {
      Linking.canOpenURL(item.pdf).then(supported => {
        if (supported) {
      ...
      <FlatList
        ...
        renderItem={({item, index}) => (
          <View key={index}>
            <View style={{flexDirection: 'row'}}>
              ...
              <Link
                ...
                <Card style={{minHeight: 50, justifyContent: 'center'}}>
                  <Title numberOfLines={2}>{item.append_title}</Title>
                </Card>
              </Link>
            </View>
            ...
            <Modal
              isVisible={modal}
              style={{marginTop: 40, backgroundColor: '#fff', margin: 0}}>
              <WebView
                source={{html: text}}
